...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Fixed parameter order: name, email, password, role
      const success = await signup(name, email, password, role as 'customer' | 'provider');
      if (success) {
        if (role === 'customer') {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      const success = await googleLogin(role as 'customer' | 'provider');
      if (success) {
        if (role === 'customer') {
        navigate('/customerDashboard');
      } else {
      ...
      toast({ 
        title: "Error", 
        description: "Google signup error occurred.", 
        variant: "destructive" 
      });
    }
  };
  ...
  const handleFacebookSignup = async () => {
    ...
    try {
      const success = await facebookLogin(role as 'customer' | 'provider');
      if (success) {
        if (role === 'customer') {
        navigate('/customerDashboard');
      } else {
      ...
      toast({ 
        title: "Error", 
        description: "An error occurred during Facebook signup.", 
        variant: "destructive" 
      });
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select 
                  ...
                  required 
                  className="w-full border border-gray-300 rounded-md h-12 px-3 focus:outline-none focus:ring-2 focus:ring-[#ff00c8] focus:border-transparent"
                >
